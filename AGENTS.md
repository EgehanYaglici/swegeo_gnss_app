# AGENTS.md — SWEGEO GNSS App (Production)
...
1. **Source Agnostic**: `MessageRouter` maps heterogeneous sources (BYNAV Binary, NMEA, ASCII) into standardized "Capability" objects using `field_mapping` from `display_config.json5`.
2. **IPC Bridging**: `main.js` translates backend events into localized IPC channels (`data:position`, `terminal:line`).
3. **GGA Loop**: Main process auto-feeds GGA strings from serial to NTRIP client when connected.
...
- **General Understanding**: See `C:\Users\egeha\.gemini\antigravity\brain\12c94b6d-0fc9-4b77-a131-f877e66af139\project_mapping.md` for a deep-dive into active vs. unused files and core logic.
- **Manual Release Ritual**: Always fix the hyphen-to-dot issue in `latest.yml` before publishing (see Release section below).
...
**Build sorunlari**:
- `signAndEditExecutable: false` OLMAMALI — bu satir exe ikonunu engeller, package.json'dan kaldirildi
- NSIS sadece `.ico` kabul eder, `.png` ile hata verir
- `${GH_TOKEN}` package.json'da OLMAMALI — electron-builder env'den otomatik okur

**ICO dosyasi olusturma** (swegeo_logo_preview.png kaynaktan):
```python
from PIL import Image
import struct, io, os

src = Image.open('assets/swegeo_logo_preview.png').convert('RGBA')
...
offset = header_size
entries = [(s, pngs[i], offset + sum(len(pngs[j]) for j in range(i))) for i, s in enumerate(sizes)]

with open('assets/swegeo_logo.ico', 'wb') as f:
    f.write(struct.pack('<HHH', 0, 1, num))
    for (s, png_data, off) in entries:
        w = s if s < 256 else 0
        f.write(struct.pack('<BBBBHHII', w, w, 0, 0, 1, 32, len(png_data), off))
    for (_, png_data, _) in entries:
        f.write(png_data)
```
ICO 7 boyut icermeli, hepsi PNG sikiştirmali olmali (toplam ~16KB).
...
# 7. latest.yml icindeki URL'leri duzelt (KRITIK — her build sonrasi zorunlu)
# electron-builder hyphens yazar: SWEGEO-GNSS-Monitor-Setup-0.2.0.exe
# GitHub assets dots kullanir:   SWEGEO.GNSS.Monitor.Setup.0.2.0.exe
# dist/latest.yml icinde "url:" ve "path:" satirlarini elle duzelt:
#   SWEGEO-GNSS-Monitor-Setup-0.2.0.exe  ->  SWEGEO.GNSS.Monitor.Setup.0.2.0.exe

# 8. Build dosyalarini release'e yukle
...
2. Test et (npm start)
3. package.json version alanini guncelle (manuel edit)
4. git add <degisen dosyalar> && git commit -m "feat: vX.Y.Z - aciklama"
5. git tag vX.Y.Z
6. git push origin main --tags
7. npm run build
8. dist/latest.yml icindeki URL'leri duzelt (hyphen -> dot)
   SWEGEO-GNSS-Monitor-Setup-X.Y.Z.exe -> SWEGEO.GNSS.Monitor.Setup.X.Y.Z.exe
9. gh release create vX.Y.Z --title "vX.Y.Z" --notes "- Degisiklik 1\n- Degisiklik 2"
10. gh release upload vX.Y.Z "dist/SWEGEO GNSS Monitor Setup X.Y.Z.exe" "dist/latest.yml" "dist/SWEGEO GNSS Monitor Setup X.Y.Z.exe.blockmap" --clobber
11. Kullanicilar otomatik guncelleme alir
```

**KRITIK**: latest.yml URL duzeltmesi yapilmazsa auto-updater 404 alir ve guncelleme basarisiz olur.
...
| Symlink hatasi | `"signAndEditExecutable": false` ekle (ancak bu exe ikonunu bozar!) |
| Exe ikonu Electron default gorunuyor | `"signAndEditExecutable": false` satirini package.json'dan kaldir |
| Windows ikonunu guncellemiyor | Windows icon cache sorunu — PC yeniden baslatinca duzenir |
| Update error (dev mode) | `app.isPackaged` false oldugu icin updater calismaz, normal |
| Update bulamiyor (404) | latest.yml'deki URL'ler hyphen degil dot olmali: `SWEGEO.GNSS.Monitor.Setup.X.Y.Z.exe` |
| Update bulamiyor (genel) | Release'de `latest.yml` + `.exe` + `.blockmap` olmali; repo PUBLIC olmali |
| Private repo'da update calismaz | GitHub release assets private repo'da anonim erisilemiyor — repo public olmali |
| Pre-release guncelleme gelmiyor | Normal — sadece "Latest" (pre-release degil) release kontrol edilir |
| Installer bulanik | `build/installer.nsh` dosyasinda `ManifestDPIAware true` olmali |
| ICO gecersiz format | ICO'yu Python Pillow ile PNG compression ile olustur (skripti yukarda) |
